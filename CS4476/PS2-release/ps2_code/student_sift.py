...
def get_magnitudes_and_orientations(dx, dy):
    ...
    # magnitudes = np.sqrt(dx**2 + dy**2).astype(np.float32)
    # orientations = np.arctan2(dy, dx).astype(np.float32)
...
def get_feat_vec(x,y,magnitudes, orientations,feature_width):
    ...
    if r0 < 0 or c0 < 0 or r1 > H or c1 > W:
        return np.zeros(g * g * 8, dtype=np.float64)

    mag_patch = magnitudes[r0:r1, c0:c1]
    ang_patch = orientations[r0:r1, c0:c1]
    ...
    for by in range(g):
        for bx in range(g):
            ...
            cc0 = bx * cell
            cc1 = cc0 + cell
            cell_mag = mag_patch[rr0:rr1, cc0:cc1].reshape(-1)
            cell_ang = ang_patch[rr0:rr1, cc0:cc1].reshape(-1)
            hist, _ = np.histogram(cell_ang, bins=8, range=(-np.pi, np.pi), weights=cell_mag)    
            fv_parts.append(hist.astype(np.float64))

    fv = np.concatenate(fv_parts, axis=0).astype(np.float64)
...
def get_features(image, x, y, feature_width):
    ...
    fdim = 4 * 4 * 8
    fvs = np.zeros((k, fdim), dtype=np.float32)
    for i, (xi, yi) in enumerate(zip(x, y)):
        fvs[i, :] = get_feat_vec(xi, yi, magnitudes, orientations, feature_width).astype(np.float64)
