...
class Prob5():
    def __init__(self):
        ...
        self.blur_filter = np.ones((9,9)) / 81.
        ...
        ###### START CODE HERE ######
        self.cat = io.imread('cat.bmp').astype(np.float64) / 255.0
        self.dog = io.imread('dog.bmp').astype(np.float64) / 255.0
        ###### END CODE HERE ######
    ...
    def my_conv2d(self, image, filter):
        ...
        Args:
            image: an RGB image array of size H x W x 3
            filter: a 2D filter array of size M x N
        
        Returns:
            convolved_image: the image with the filter applied to each channel (size H x W x 3)
        """
        ...
        for channel in range(channels):
            out[:, :, channel] = convolve(image[:, :, channel], filter, mode='reflect')
        convolved_image = np.clip(out, 0.0, 1.0)
        ###### END CODE HERE ######
        return convolved_image


    def prob_5_1(self):
        """Apply the identity filter, blur filter, sobel filter, and laplacian filter (provided as class variables)
        to the cat image (self.cat). Return each result image. Display them in your report.
        
        Returns:
            identity_image: the cat image with the identity filter applied (size H x W x 3)
            blur_image: the cat image with the blur filter applied (size H x W x 3)
            sobel_image: the cat image with the sobel filter applied (size H x W x 3)
            laplacian_image: the cat image with the laplacian filter applied (size H x W x 3)
        """
    ...
    def create_hybrid_image(self, image1, image2):
        ...
        Args:
            image1: an RGB image array of size H x W x 3
            image2: an RGB image array of size H x W x 3

        Returns:
            hybrid_image: the hybrid image of image1 and image2 (size H x W x 3)
        """
        hybrid_image = None
        ###### START CODE HERE ######
        low_freq1 = self.my_conv2d(image1, self.gaussian_filter)
        low_freq2 = self.my_conv2d(image2, self.gaussian_filter)
        high_freq2 = image2 - low_freq2
        hybrid = low_freq1 + high_freq2
        hybrid_image = np.clip(hybrid, 0.0, 1.0).astype(np.float32)
        ###### END CODE HERE ######
        return hybrid_image
    ...
    def prob_5_2(self):
        ...
        Returns:
            hybrid_image: the hybrid image created from the dog and cat images (size H x W x 3)
